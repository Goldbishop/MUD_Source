    ...
    if (isname (name, i->player.name) && CAN_SEE (ch, i)) {
      if ((i->in_room != NOWHERE) && ((GET_LEVEL (ch) > 20) ||
        ...
        if (IS_NPC (i))
          ...
          sprintf (buf, "%-30s- %s ", i->player.name, world[i->in_room].name);
  ...
  *buf = '\0';
  ...
  for (i = 1; i < 21; i++) {
    sprintf (buf + strlen (buf), "%7d-%-7d : ",
      titles[class - 1][i].exp, titles[class - 1][i + 1].exp);
    switch (GET_SEX (ch)) {
    case SEX_MALE:
      strcat (buf, titles[class - 1][i].title_m);
      break;
    case SEX_FEMALE:
      strcat (buf, titles[class - 1][i].title_f);
      break;
    default:
