  ...
  struct char_data *ch)
{
  ...
  if (obj) {
    ...
    if ((obj->obj_flags.timer < 0) && (obj->obj_flags.timer != OBJ_NOTIMER)) {
      sprintf (buf,
        "You're told: 'The %s is just old junk, I'll throw it away for you.'\n\r",
        fname (obj->name));
      send_to_char (buf, ch);
    } else {
...
int receptionist (struct char_data *ch, int cmd, char *arg)
{
  ...
  if (!CAN_SEE (recep, ch)) {
    act ("$n says, 'I don't deal with people I can't see!'", FALSE, recep, 0,
      0, TO_ROOM);
    return (TRUE);
  }
  ...
  if (cmd == 92) {              /* Rent  */
    if (recep_offer (ch, recep, &cost)) {
      ...
      act
        ("$n stores your stuff in the safe, and helps you into your chamber.",
